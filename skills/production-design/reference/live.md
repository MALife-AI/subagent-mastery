...
```bash
node .agents/skills/production-design/scripts/live.mjs
```
...
```
LOOP:
  node .agents/skills/production-design/scripts/live-poll.mjs   # default long timeout; no --timeout=
...
```bash
node .agents/skills/production-design/scripts/live-wrap.mjs --id EVENT_ID --count EVENT_COUNT --element-id "ELEMENT_ID" --classes "class1,class2" --tag "div"
```
...
```bash
node .agents/skills/production-design/scripts/live-poll.mjs --reply EVENT_ID done --file RELATIVE_PATH
```
...
```bash
node .agents/skills/production-design/scripts/live-server.mjs stop
```
...
```bash
node .agents/skills/production-design/scripts/detect-csp.mjs
```
